...
const AuditDetailsPage = () => {
  ...
  const [reportData, setReportData] = useState({
    ...
    overallRisk: "Medium",
    auditScore: String(MOCK_AUDITS.find(a => a.auditId === auditId)?.auditScore ?? 75.25),
    auditRating: MOCK_AUDITS.find(a => a.auditId === auditId)?.auditRating ?? "Good (B)",
  ...
  const [processRows, setProcessRows] = useState([
    { process: "Sourcing, Underwriting & Disbursement", checkpoints: "42", compliant: "35", nonCompliant: "7", score: "84", owner: "Sales / Credit" },
    { process: "Collection & Recovery", checkpoints: "18", compliant: "14", nonCompliant: "4", score: "78", owner: "Collection" },
    { process: "In-Branch Audit", checkpoints: "26", compliant: "22", nonCompliant: "4", score: "86", owner: "Operations" },
  ]);
  const [checkpointRows, setCheckpointRows] = useState(() => BRD_CHECKPOINTS.map((cp, i) => ({
    process: cp.process,
  ...
  const [auditTrail, setAuditTrail] = useState<{ timestamp: string; user: string; role: string; action: string; details?: string }[]>([
  ...
  const [editableObservations, setEditableObservations] = useState(() => {
    ...
    return (records.length ? records : [
      { issueId: "DRAFT-01", issueCategory: "Lead information", observation: "Sector wrongly updated", issueSeverity: "Medium", riskClassification: "Process non compliance risk", department: "Sales", issueStatus: "Pending", valueAtRisk: 300000 },
      { issueId: "DRAFT-02", issueCategory: "Physical cash", observation: "Cash verification not done properly", issueSeverity: "High", riskClassification: "Fraud Risk", department: "Operations", issueStatus: "Pending", valueAtRisk: 500000 },
    ]).map(o => ({ ...o, valueAtRisk: String(o.valueAtRisk) }));
  });
  ...
  const updateProcessRow = (index: number, field: keyof (typeof processRows)[number], value: string) => {
    setProcessRows(prev => prev.map((row, rowIndex) => rowIndex === index ? { ...row, [field]: value.slice(0, 120) } : row));
  };
  ...
  const addCheckpointRow = () => {
    setCheckpointRows(prev => [...prev, {
      process: "",
      checkpointCode: `CHK-${String(prev.length + 1).padStart(2, "0")}`,
      accountRef: "",
      checkpoint: "",
      expectedControl: "",
      sampleSize: "0",
  ...
  const addObservationRow = () => {
    setEditableObservations(prev => [...prev, {
      issueId: `DRAFT-${String(prev.length + 1).padStart(2, "0")}`,
      issueCategory: "",
      observation: "",
                ...
                <div className="space-y-6">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <Button size="sm" disabled={reportLocked} onClick={() => { logTrail("Draft saved", `Score ${reportScore}%, ${checkpointRows.length} checkpoints`); toast({ title: "Draft saved", description: `${audit.auditId} report draft saved.` }); }}>
                  ...
                  <div className="rounded-lg border overflow-hidden">
                    <div className="bg-muted/40 px-4 py-3 border-b">
                      <h3 className="text-sm font-semibold">Report Header & Audit Summary</h3>
                    </div>
                      ...
                      <table className="w-full text-sm">
                        ...
                        <tbody className="divide-y">
                          {processRows.map((row, index) => (
                            <tr key={row.process}>
                              <td className="px-4 py-3 min-w-64"><Input value={row.process} onChange={e => updateProcessRow(index, "process", e.target.value)} /></td>
                  ...
                  <div className="rounded-lg border overflow-hidden">
                    <div className="flex flex-col gap-3 bg-muted/40 px-4 py-3 border-b sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <h3 className="text-sm font-semibold">Detailed Audit Checklist - Individual Checkpoints</h3>
                        ...
                        <tbody className="divide-y">
                          {checkpointRows.map((row, index) => (
                            <tr key={`${row.checkpointCode}-${index}`} className={row.mandatory && row.result === "Pending" ? "bg-destructive/5" : ""}>
                              <td className="px-4 py-3 min-w-44"><Input disabled={reportLocked} value={row.process} onChange={e => updateCheckpointRow(index, "process", e.target.value)} /></td>
                  ...
                  <div className="rounded-lg border overflow-hidden">
                    <div className="bg-muted/40 px-4 py-3 border-b">
                      <h3 className="text-sm font-semibold">Score, Rating & Management Response</h3>
                    </div>
                  ...
                  <div className="rounded-lg border overflow-hidden">
                    <div className="flex flex-col gap-3 bg-muted/40 px-4 py-3 border-b sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <h3 className="text-sm font-semibold">5.4.2.a. Issues Summary</h3>
                        <p className="text-xs text-muted-foreground">Complied: severity-wise issue count, open/closed status and value at risk are available for testing.</p>
                      </div>
                        ...
                        <tbody className="divide-y">
                          {editableObservations.map((observation, index) => (
                            <tr key={`${observation.issueId}-${index}`}>
                              <td className="px-4 py-3 min-w-28"><Input value={observation.issueId} onChange={e => updateObservationRow(index, "issueId", e.target.value)} /></td>
                              <td className="px-4 py-3 min-w-44"><Input value={observation.issueCategory} onChange={e => updateObservationRow(index, "issueCategory", e.target.value)} /></td>
                              ...
                              <td className="px-4 py-3 min-w-56"><Input value={observation.riskClassification} onChange={e => updateObservationRow(index, "riskClassification", e.target.value)} /></td>
                              <td className="px-4 py-3 min-w-40"><Input value={observation.department} onChange={e => updateObservationRow(index, "department", e.target.value)} /></td>
                              ...
                              <td className="px-4 py-3 min-w-36"><Select value={observation.issueStatus} onValueChange={value => updateObservationRow(index, "issueStatus", value)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="Pending">Pending</SelectItem><SelectItem value="Accept">Accept</SelectItem><SelectItem value="Reject">Reject</SelectItem><SelectItem value="Sent Back">Sent Back</SelectItem><SelectItem value="Overdue">Overdue</SelectItem><SelectItem value="Closed">Closed</SelectItem></SelectContent></Select></td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
