...
// Mutating helpers so admin updates propagate to all importers within the session.
const replaceList = (target: string[], next: string[]) => {
  target.splice(0, target.length, ...next);
};
export const setProcessOptions = (next: string[]) => replaceList(PROCESS_OPTIONS, next);
export const setRiskClassifications = (next: string[]) => replaceList(RISK_CLASSIFICATIONS, next);
export const setDepartments = (next: string[]) => replaceList(DEPARTMENTS, next);
export const setIssueCategories = (next: string[]) => replaceList(ISSUE_CATEGORIES, next);
