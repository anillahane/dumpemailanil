...
export const SEVERITY_VALUES: BrdCheckpoint["defaultSeverity"][] = ["Low", "Medium", "High", "Critical"];
...
// Mutate the shared library in place so all importers see updates within a session.
export const replaceBrdCheckpoints = (next: BrdCheckpoint[]) => {
  BRD_CHECKPOINTS.splice(0, BRD_CHECKPOINTS.length, ...next);
};
