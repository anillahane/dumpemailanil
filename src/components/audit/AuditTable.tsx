              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  <div className="min-w-0">
                    <p className="font-mono text-sm font-medium text-primary">{audit.auditId}</p>
                    <p className="text-sm font-medium text-primary mt-0.5">{audit.branchName}</p>
                    <p className="text-xs text-muted-foreground">{audit.auditType}</p>
                  </div>
                  <Badge variant="outline" className={`text-[10px] font-medium shrink-0 ${badge.className}`}>
                    {badge.label}
                  </Badge>
                </div>
                <div className="grid grid-cols-2 gap-2 text-xs mt-3">
                  <div>
                    ...
                    <p className="font-medium">{audit.auditorId}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Period</p>
                    <p className="font-medium">{audit.auditPeriodFrom} – {audit.auditPeriodTo}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Start</p>
                    <p className="font-medium">{audit.auditStartDate}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">End</p>
                    <p className="font-medium">{audit.auditEndDate}</p>
                  </div>
                  {showScore && (
                    <div>
                      <p className="text-muted-foreground">Score</p>
                      <p className="font-medium">{audit.auditScore ? `${audit.auditScore}%` : "—"}</p>
                    </div>
                  )}
                  {showDaysLeft && (
                    <div>
                      <p className="text-muted-foreground">Days Left</p>
                      <p className={`font-medium ${(audit.daysLeft ?? 0) <= 1 ? "text-destructive" : "text-foreground"}`}>
                        {audit.daysLeft ?? "—"}
                      </p>
                    </div>
                  )}
